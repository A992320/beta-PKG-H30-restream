...
if (session_status() === PHP_SESSION_NONE && PHP_SAPI !== 'cli') {
    ...
    /* جلسات الإدارة تحفظ في مجلد النظام نفسه كي لا يحذفها موقع آخر على
       الاستضافة المشتركة. تبقى الجلسة سبعة أيام ما دام المتصفح مفتوحاً؛
       الكعكة نفسها تبقى جلسية وتنتهي عند إغلاق المتصفح. */
    $__sessionTtl = max(7200, min(2592000, (int) env('ADMIN_SESSION_TTL', 604800)));
    $__sessionDir = dirname(__DIR__) . '/storage/sessions';
    if (is_dir($__sessionDir) || @mkdir($__sessionDir, 0700, true)) {
        @chmod($__sessionDir, 0700);
        if (is_writable($__sessionDir)) @session_save_path($__sessionDir);
    }
    ini_set('session.gc_maxlifetime', (string) $__sessionTtl);
