...
function initStream(url,subUrl){
  ...
  /* ══ توجيه حسب الصيغة — باختيار المستخدم ══
     m3u8 و ts  → عبر الوسيط (حين يكون مفعّلاً ولدينا مرجع).
     mp4 و mkv → مباشرةً دائماً، بلا وسيط.

     الحارس PL._tsRestreamTried يمنع الحلقة: حين يعيد الوسيط رابط m3u8
     ويُعاد إلى initStream، يكون العلَم مضبوطاً، فلا يُعاد توجيهه ويُشغَّل
     مباشرةً عبر hls.js. */
  if((fmt==='hls' || fmt==='ts')
    ...
    showBuf(true);
    if(typeof toast==='function') toast('جارٍ التشغيل عبر الخادم…');
