...
(function () {
  ...
  var restreamPing = null, restreamTries = 0, restreamRevives = 0, restreamActive = false;
   ...
   */
  function tryRestream(sref, codec) {
    fetch('restream.php?' + sref.q + '=' + sref.id, { credentials: 'same-origin' })
      ...
      .then(function (res) {
        ...
        if (res.s === 202 && j.error === 'starting') {
          if (++restreamTries <= 8) {
            say('جارٍ تجهيز الصوت…');
            setTimeout(function () { tryRestream(sref, codec); }, (j.retry_after || 2) * 1000);
          } else {
        ...
        var H = hls(), v = vid();
        if (!H || !v) { plainAudioWarning(codec); return; }
        try {
          H.loadSource(j.url);
          H.startLoad();
          v.play().catch(function () {});
          restreamActive = true;
          say(codec === 'container' ? 'جارٍ التشغيل عبر الخادم' : 'أُصلح الصوت — تحويل على الخادم');
        } catch (e) { plainAudioWarning(codec); return; }
          ...
          fetch('restream.php?' + sref.q + '=' + sref.id + '&ping=1', { credentials: 'same-origin' })
            .then(function (pr) {
              ...
              if (pr.status === 410 && restreamRevives < 3) {
                ...
                tryRestream(sref, 'revive');
              }
            })
            .catch(function () {});
        }, 25000);
      })
      .catch(function () { plainAudioWarning(codec); });
  }
  ...
  function warnAudio(codec) {
    ...
    if (sref.id > 0) { tryRestream(sref, codec); return; }
    plainAudioWarning(codec);
  }
    ...
    if (H && H.on && !H._ppHooked) {
      ...
      try {
        ...
        H.on(window.Hls.Events.ERROR, function (e, d) {
          ...
          if (det === 'bufferAddCodecError' || det === 'bufferIncompatibleCodecsError') {
            var mt = (d.mimeType || d.err && d.err.message || '');
            if (/audio/i.test(mt) || UNPLAYABLE_AUDIO.test(mt)) warnAudio(mt || 'غير معروف');
          }
        });
      } catch (e) {}
    }
