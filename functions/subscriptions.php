...
function subsEnsureSchema(): bool
{
    ...
    if (is_file($flag)) {
        // لا نثق بعلامة قديمة وحدها: قد تبقى بعد ترقية ناقصة أو استعادة نسخة.
        // نتحقق من الجداول الأساسية قبل استعمالها، ثم نعيد التهيئة عند أي نقص.
        try {
            $checkPdo = db();
            foreach (['subscription_plans','site_users','coupons','coupon_redemptions','site_login_logs'] as $table) {
                $checkPdo->query("SELECT 1 FROM `{$table}` LIMIT 1");
            }
            subsEnsureDeviceSchema();
            return $done = true;
        } catch (Throwable $e) {
            @unlink($flag);
            subsSchemaNote('علامة مخطط قديمة: ' . $e->getMessage());
        }
    }
    ...
    try {
        ...
        // ترحيل الأجهزة لا يعطّل إدارة الخطط إذا احتاج صلاحية ALTER إضافية.
        subsEnsureDeviceSchema();
        ...
        foreach ($need as $tbl) {
            try {
                // فحص مباشر لا يحتاج صلاحية information_schema، المتاحة دائماً
                // لحسابات MySQL التي يمكنها قراءة جداول التطبيق.
...
function subsEnsureDeviceSchema(): bool
{
    ...
    try {
        ...
        $column = $pdo->query("SHOW COLUMNS FROM `site_users` LIKE 'max_devices'");
