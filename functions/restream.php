...
/** نافذة HLS الحي: 12 × ثانيتين = 24 ثانية حماية من اهتزاز المصدر.
 * يمكن تخصيصها من RESTREAM_LIVE_LIST_SIZE دون تخزين مقاطع غير محدودة. */
function rsLiveListSize(): int {
    return max(8, min(20, (int)rsCfg('RESTREAM_LIVE_LIST_SIZE', 12)));
}

/** لا نطلق عمليات FFmpeg الجديدة كلها في اللحظة نفسها عند فتح مجموعة. */
function rsStartGapMs(): int {
    return max(250, min(5000, (int)rsCfg('RESTREAM_START_GAP_MS', 650)));
}

/** عدد أنوية الخادم من /proc بلا استدعاء إضافي للطرفية. */
function rsCpuCores(): int {
    static $cores = null;
    if ($cores !== null) return $cores;
    $raw = @file_get_contents('/proc/cpuinfo');
    $n = is_string($raw) ? preg_match_all('/^processor\\s*:/mi', $raw, $m) : 0;
    return $cores = max(1, (int)$n);
}

/** لا تبدأ قناة باردة جديدة حين يصل حمل الخادم إلى حدّ الخطر.
 * القنوات العاملة تبقى مستقرة، والطلب الجديد يعاد بعد لحظة تلقائياً. */
function rsStartLoadAllows(): bool {
    $limit = (float)rsCfg('RESTREAM_START_MAX_LOAD', 1.35);
    if ($limit <= 0 || !function_exists('sys_getloadavg')) return true;
    $load = @sys_getloadavg();
    if (!is_array($load) || !isset($load[0]) || !is_numeric($load[0])) return true;
    return ((float)$load[0] / rsCpuCores()) < $limit;
}

/** بوابة عالمية: عملية جديدة كل 650ms افتراضياً بدل عاصفة تشغيل جماعية. */
function rsAcquireStartSlot(): array {
    if (!rsStartLoadAllows()) return ['ok' => false, 'retry_after' => 1.25];
    $dir = dirname(__DIR__) . '/storage';
    if (!is_dir($dir) && !@mkdir($dir, 0755, true) && !is_dir($dir)) return ['ok' => true];
    $fp = @fopen($dir . '/.restream_start_gate', 'c+');
    if ($fp === false) return ['ok' => true];
    if (!@flock($fp, LOCK_EX)) { @fclose($fp); return ['ok' => true]; }
    @rewind($fp);
    $last = (float)trim((string)stream_get_contents($fp));
    $now = microtime(true);
    $gap = rsStartGapMs() / 1000;
    if ($last > 0 && ($now - $last) < $gap) {
        $retry = max(0.25, $gap - ($now - $last));
        @flock($fp, LOCK_UN); @fclose($fp);
        return ['ok' => false, 'retry_after' => round($retry, 2)];
    }
...
function rsPipelineVersion(): string { return 'hls-browser-fmp4-v9-group-stable'; }
...
function rsReady(string $key): bool {
    $idx = rsIndex($key);
    $dir = rsDir($key);
    // لا يكفي وجود القائمة وحده؛ نتحقق من ملف التهيئة وأول مقطع فعلي.
    if (!is_file($idx) || (int)@filesize($idx) <= 40) return false;
    if (!is_file($dir . '/init.mp4') || (int)@filesize($dir . '/init.mp4') < 100) return false;
    foreach (glob($dir . '/s*.m4s') ?: [] as $segment) {
        if ((int)@filesize($segment) > 1024) return true;
    }
    return false;
}
...
function rsStart(string $key, string $srcUrl, bool $compatVideo = false, string $audioUrl = '', float $audioDelay = 0.0): array {
    ...
    /* بوابة بدء جماعية: لا نستهلك المعالج/الشبكة بإقلاع كل قنوات المجموعة معاً.
       لا نحتجز الطلب؛ الواجهة تعيد السؤال بعد المهلة المحددة. */
    $slot = rsAcquireStartSlot();
    if (empty($slot['ok'])) {
        flock($lock, LOCK_UN); fclose($lock);
        return ['ok' => true, 'pending' => true, 'queued' => true,
                'retry_after' => (float)($slot['retry_after'] ?? 0.65)];
    }
    ...
    $cmd = $timeout . 'ffmpeg -hide_banner -loglevel error -nostdin'
         ...
         . ' -fflags +genpts+discardcorrupt -copyts -start_at_zero -analyzeduration 1000000 -probesize 65536'
         ...
         . ' -thread_queue_size 2048 -user_agent ' . escapeshellarg('VLC/3.0.20 LibVLC/3.0.20')
         ...
         . ($audioUrl !== ''
             ? ' -itsoffset ' . escapeshellarg(number_format($audioDelay, 3, '.', '')) . ' -thread_queue_size 2048 -rw_timeout 15000000 -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -i ' . escapeshellarg($audioUrl) . ' -map 0:v:0 -map 1:a:0?'
             : ' -map 0:v:0 -map 0:a:0?')
         ...
         . ' -vsync 0 -max_interleave_delta 0 -muxdelay 0 -max_muxing_queue_size 2048 -c:a aac -b:a 192k -ac 2 -ar 48000 -af ' . escapeshellarg('aresample=48000:async=1000:min_comp=0.001:min_hard_comp=0.020:comp_duration=0.50')
         ...
         . ' -f hls -hls_segment_type fmp4 -hls_time 2 -hls_init_time 1'
         . ' -hls_fmp4_init_filename init.mp4'
         /* ══ الفارق الجوهري بين الفيلم والبثّ الحيّ ══
            الحيّ: قائمة متدحرجة من اثني عشر مقطعاً تُحذف خلف المشاهد. لا معنى
         ...
         . ($isVod
             ...
             : (' -hls_list_size ' . rsLiveListSize() . ' -hls_flags independent_segments+delete_segments+append_list+omit_endlist+temp_file'))
         . ' -hls_allow_cache 0'
